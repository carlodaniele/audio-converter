...
PLUGIN_DIR="$ROOT_DIR/plugins/nomad-pipeline-audio-to-draft"
...
ZIP_NAME="nomad-pipeline-audio-to-draft-${STABLE_TAG}.zip"
...
rsync -a \
  ...
  "$PLUGIN_DIR/" "$TMP_DIR/nomad-pipeline-audio-to-draft/"

# Defensive cleanup for macOS metadata in case files are created after sync.
find "$TMP_DIR/nomad-pipeline-audio-to-draft" \( -name '.DS_Store' -o -name '._*' \) -delete

( cd "$TMP_DIR" && zip -rq "$ZIP_PATH" "nomad-pipeline-audio-to-draft" )
